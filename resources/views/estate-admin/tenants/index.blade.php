...
<div class="mb-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-600 mt-1">Manage tenants in {{ $estate->name }}</p>
        </div>
        <div>
            <a href="{{ route('estate.tenants.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition text-sm font-medium">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <div class="text-sm">{{ $tenant->lease_start_date->format('M d, Y') }} - {{ $tenant->lease_end_date->format('M d, Y') }}</div>
                        <div class="text-xs text-gray-500">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <a href="{{ route('estate.tenants.show', $tenant) }}" class="text-indigo-600 hover:text-indigo-800 mr-3">View</a>
                        <a href="{{ route('estate.tenants.edit', $tenant) }}" class="text-blue-600 hover:text-blue-800 mr-3">Edit</a>
                        <form action="{{ route('estate.tenants.destroy', $tenant) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-red-600 hover:text-red-800" onclick="return confirm('Are you sure you want to remove this tenant?')">Delete</button>
                        </form>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="8" class="px-6 py-12 text-center">
                        ...
                        <div class="mt-6">
                            <a href="{{ route('estate.tenants.create') }}" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 inline-block">
                                Add New Tenant
                            </a>
                        </div>
                    </td>
