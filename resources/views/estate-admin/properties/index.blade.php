...
<div class="mb-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-600 mt-1">Manage properties in {{ $estate->name }}</p>
        </div>
        <div class="flex items-center space-x-3">
            ...
            <a href="{{ route('estate.properties.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition text-sm font-medium">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                <div class="text-sm font-medium text-gray-900">{{ $property->property_name }}</div>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {{ $property->bedrooms_per_unit ?? 0 }}BR / {{ $property->bathrooms_per_unit ?? 0 }}BA
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm font-semibold text-gray-900">₦{{ number_format($property->rent_amount_per_unit, 2) }}</div>
                        <div class="text-xs text-gray-500">{{ ucfirst($property->rent_period) }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            <div class="w-16 bg-gray-200 rounded-full h-2 mr-2">
                                <div class="bg-blue-600 h-2 rounded-full" style="width: {{ $property->getOccupancyRate() }}%"></div>
                            </div>
                            <span class="text-xs font-medium">{{ $property->occupied_units }}/{{ $property->units }}</span>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <a href="{{ route('estate.properties.edit', $property) }}" class="text-blue-600 hover:text-blue-800 mr-3">Edit</a>
                ...
                <tr>
                    <td colspan="9" class="px-6 py-12 text-center">
                        ...
                        <div class="mt-6">
                            <a href="{{ route('estate.properties.create') }}" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 inline-block">
                                Add New Property
                            </a>
                        </div>
                    </td>
...
<div id="grid-view" class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 hidden">
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-md transition">
        ...
        <div class="p-6">
            ...
            @if($property->bedrooms_per_unit || $property->bathrooms_per_unit)
            <div class="grid grid-cols-2 gap-4 mb-4">
                @if($property->bedrooms_per_unit)
                <div>
                    <p class="text-xs text-gray-500 uppercase">Bedrooms/Unit</p>
                    <p class="text-sm font-medium text-gray-900">{{ $property->bedrooms_per_unit }} BR</p>
                </div>
                @endif
                @if($property->bathrooms_per_unit)
                <div>
                    <p class="text-xs text-gray-500 uppercase">Bathrooms/Unit</p>
                    <p class="text-sm font-medium text-gray-900">{{ $property->bathrooms_per_unit }} BA</p>
                </div>
                @endif
            </div>
            @endif
            ...
            <div class="mb-4">
                ...
                <p class="text-sm font-medium text-gray-900">{{ number_format($property->size_sqm, 2) }} {{ $property->size_unit ?? 'sqm' }}</p>
            </div>
            @endif
            ...
            <div class="mb-4 p-3 bg-gray-50 rounded-lg">
                <div class="flex items-center justify-between mb-2">
                    ...
                    <span class="text-xs font-semibold text-gray-900">{{ $property->occupied_units }}/{{ $property->units }} Units</span>
                </div>
            ...
            <div class="p-4 bg-blue-50 rounded-lg mb-4">
                <p class="text-xs text-gray-600 mb-1">Rent per Unit</p>
                <p class="text-2xl font-bold text-blue-600">₦{{ number_format($property->rent_amount_per_unit, 2) }}</p>
                <p class="text-xs text-gray-600">{{ ucfirst($property->rent_period) }}</p>
                @if($property->units > 1)
                <p class="text-xs text-gray-500 mt-2">Total Potential: ₦{{ number_format($property->total_rent_potential, 2) }}</p>
                @endif
            </div>

            @if($property->description)
            <div class="mb-4">
                <p class="text-sm text-gray-600">{{ Str::limit($property->description, 100) }}</p>
            </div>
            @endif
        </div>

        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
            <a href="{{ route('estate.properties.edit', $property) }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                Edit Property
            </a>
            <form action="{{ route('estate.properties.destroy', $property) }}" method="POST" class="inline">
                @csrf
    ...
    <div class="col-span-full">
        <div class="bg-white rounded-lg shadow-sm p-12 text-center">
            ...
            <div class="mt-6">
                <a href="{{ route('estate.properties.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const currentView = localStorage.getItem('propertyViewPreference') || 'list';
